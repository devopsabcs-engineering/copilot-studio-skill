...
* DD-10: Lab 05 documented the `@copilot-studio:copilot-studio-*` sub-agent autocomplete surface as the CLI verification path; live `copilot` CLI does not expose sub-agents that way.
  * Research recommends: N/A directly — this is a plan-vs-reality deviation surfaced post-handoff during interactive maker testing (iteration 4).
  * Plan implements: Phase 2 authored Lab 05 (EN + FR) with Exercise 5.2 instructing the maker to type a bare `@` at the `copilot` prompt and expect autocomplete to list `@copilot-studio:copilot-studio-manage`, `-author`, `-test`, and `-advisor`. Observed CLI behavior (verified via maker screenshot): (a) install confirmation reads `Installed 31 skills. Use /skills list to see them.`, (b) bare `@` triggers **file** completion (paths in the workspace, not agents), (c) the plugin registers 31 skills plus four sub-agents that are addressed via the `/agent` slash command picker (or `--agent=<slug>` on the command line, or natural-language reference inside a prompt), not via the `@`-autocomplete pattern documented in the Claude-centric SETUP_GUIDE.
  * Rationale: research and plan treated the Claude Code autocomplete pattern as the CLI surface; GitHub Copilot CLI uses different invocation mechanics. Skills with `user-invocable: false` are auto-loaded by description match against natural-language prompts; sub-agents are pinned via `/agent`. The four-role distinction is structurally real (separate `.md` files in `agents/`), just reached differently per host.
  * Impact: medium (Lab 05 verification step did not match live behavior; learners would have failed the verification checkpoint).
  * **Resolution (iteration 4):** Lab 05 EN + FR Overview rewritten to describe install confirmation, 31-skill catalog, four logical roles, and two surfaces (CLI = `/skills list` + `@copilot-studio` natural language; VS Code = friendly-named agents in picker). Exercise 5.2 changed to verify via `/skills list`; new optional 5.3 added covering VS Code picker. Resolution was directionally correct but incomplete — did not show the maker how to actually USE a skill or pin a sub-agent in the CLI.
  * **Resolution (iteration 5, applied):** Lab 05 EN + FR rewritten again to (a) remove the inaccurate "`@copilot-studio` is the single CLI-addressable agent" framing, (b) document the actual CLI mechanics explicitly (skills auto-load on description match because of `user-invocable: false`; sub-agents reached via `/agent` slash picker, `--agent=<slug>` flag, or natural-language reference), (c) add Exercise 5.3 — a hands-on smoke test driving the `list-kinds` skill via natural language ("List all available kinds in the Copilot Studio YAML schema") which runs the bundled `schema-lookup.bundle.js` against a local schema with no auth and no cloned agent, (d) add Exercise 5.4 — pin the **Copilot Studio Advisor** sub-agent via `/agent` and ask it to "Look up the SendActivity schema definition" so the maker sees the `lookup-schema` skill execute under the sub-agent, (e) renumber the optional VS Code chat picker step to Exercise 5.5, (f) update Verification Checkpoint to seven boxes including the two new "did the skill actually run" gates. Duration bumped from 5 min to 10 min. Investigation reference: `microsoft/skills-for-copilot-studio` SETUP_GUIDE.md + GitHub Copilot CLI docs at `https://docs.github.com/en/copilot/how-tos/use-copilot-agents/use-copilot-cli`.
...
* WI-13: Capture four Lab 05 screenshots — `images/lab-05/lab-05-skills-list.png` (CLI `/skills list` output showing 31 skills grouped by `manage`/`author`/`test`/`advisor` sub-agent), `images/lab-05/lab-05-list-kinds.png` (CLI response after natural-language prompt "List all available kinds in the Copilot Studio YAML schema" showing categorized YAML `kind` values from the `list-kinds` skill), `images/lab-05/lab-05-advisor-lookup-schema.png` (CLI with **Copilot Studio Advisor** pinned via `/agent` after asking "Look up the SendActivity schema definition", showing the `lookup-schema` skill output), and `images/lab-05/lab-05-vscode-agent-picker.png` (VS Code chat agent picker with four Copilot Studio entries) (medium priority — lab text already verifies via prose, images make the surface mechanics obvious to learners).
  * Source: Post-handoff iteration 4 (added `skills-list` + `vscode-agent-picker` references when the autocomplete pattern was removed) and iteration 5 (added `list-kinds` + `advisor-lookup-schema` references when Exercises 5.3 and 5.4 were added to demonstrate real skill/sub-agent usage in Copilot CLI).
  * Dependency: maker has the plugin installed in `copilot` CLI and the VS Code extension active (Labs 02 + 05 complete). Iteration-5 captures require running the actual exercises live — `list-kinds` and `lookup-schema` both run fully offline against the bundled JSON schema (no tenant, no cloned agent).
